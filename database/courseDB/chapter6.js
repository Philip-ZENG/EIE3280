...
const page614 = new Page({
  ...
  content: "Vi of candidate i: the number of voters who have put this candidate\
  at the first position of their lists\
  ▶ The candidate with the largest Vi will be put at the first position on\
  the output list\
  ▶ The candidate with the nth largest Vi will be put at the nth position\
  on the output list"
});
...
const page628 = new Page({
    ...
    ▶ When we have A > B, we also need to count the number of candidates\
    between A and B (called intensity)\
    ▶ IIIA: in the outcome list, the ranking of a pair of candidates depends\
    only on the pairwise comparison and the intensity.\
...
const page633 = new Page({
    pageID: 633,
    title: "Summary",
    content: "Wikipedia's success depends on positive network effect and good faith\
    collaboration.\
    Consensus formation by voting (and bargaining).\
    In trying to compress many rank-ordered lists into one, intuitions like\
    IIA may not hold.\
